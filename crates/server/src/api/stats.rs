...
use pt_reseeder_core::stats::reseed::{
    DashboardOverview, ReseedStatsService, SiteReseedStats, TrendPoint,
};
...
) -> Result<Json<DashboardOverview>, axum::http::StatusCode> {
...
async fn get_site_stats(
    State(state): State<AppState>,
) -> Result<Json<Vec<SiteReseedStats>>, axum::http::StatusCode> {
    let svc = ReseedStatsService::new(state.inner.db_pool.clone());
...
async fn get_user_info(
    State(state): State<AppState>,
) -> Result<Json<UserInfoAggregate>, axum::http::StatusCode> {
...
async fn get_user_stats_history(
    ...
    Json<Vec<pt_reseeder_core::stats::user_info::UserStatsHistoryPoint>>,
