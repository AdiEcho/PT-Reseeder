...
import { Badge, Card, CardContent, CardHeader, CardTitle, Separator, Skeleton, Tabs, TabsList, TabsTrigger } from '../../components/ui'
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-3">
            <span className={`inline-flex items-center gap-1.5 text-xs ${connected ? 'text-success' : 'text-muted-foreground'}`}>
...
function StatCard({
  ...
  return (
    <Card className={`border-l-4 ${accentColors[accent]}`}>
        ...
        {stats.length === 0 ? (
          ...
          <div className="overflow-x-auto border border-border rounded-lg">
            <table className="w-full border-collapse text-sm font-body">
              <caption className="sr-only">站点辅种统计</caption>
              <thead>
...
function breakerLabel(status: string): { label: string; variant: 'success' | 'destructive' | 'muted' } {
  if (status === 'tripped') return { label: '已暂停', variant: 'destructive' }
  if (status === 'ok') return { label: '正常', variant: 'success' }
    ...
    <Card>
      ...
      <CardContent>
        ...
        {info.sites.length === 0 ? (
          ...
          <div className="overflow-x-auto border border-border rounded-lg">
            <table className="w-full border-collapse text-sm font-body">
              <caption className="sr-only">各站点账号数据</caption>
              <thead>
                <tr className="bg-muted border-b border-border">
                  <Th>站点</Th>
                  <Th>上传量</Th>
