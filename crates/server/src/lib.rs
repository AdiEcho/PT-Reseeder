...
pub async fn run_server(config: AppConfig, cancel_token: CancellationToken) -> Result<BoundAddr, Box<dyn std::error::Error>> {
    // Init DB
    ...
    // Build state
    let state = state::AppState::new(pool, db_writer, config.clone(), cancel_token.clone());
