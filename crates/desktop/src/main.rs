            ...
            move |app| {
                ...
                std::thread::spawn(move || {
                    let runtime = tokio::runtime::Runtime::new()
                        .expect("failed to create desktop server runtime");
                    runtime.block_on(async move {
                        let mut config = AppConfig::load();
                        config.server_bind =
                            "127.0.0.1:0".parse().expect("valid desktop bind address");
                ...
                let url =
                    url::Url::parse(&format!("http://{addr}")).expect("valid embedded server URL");
